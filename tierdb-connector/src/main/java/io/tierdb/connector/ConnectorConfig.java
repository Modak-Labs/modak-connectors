...
public record ConnectorConfig(
    ...
    private static final String FILEIO_PREFIX = "tierdb.fileio.";

    public static ConnectorConfig fromMap(Map<String, String> config) {
        String jdbcUrl = config.get("tierdb.jdbc-url");
        if (jdbcUrl == null || jdbcUrl.isBlank()) {
            throw new IllegalArgumentException("catalog property tierdb.jdbc-url is required");
        }
        Duration pinTtl = Duration.ofSeconds(
                Long.parseLong(config.getOrDefault("tierdb.pin-ttl-seconds", "900")));
        ...
        return new ConnectorConfig(jdbcUrl, config.get("tierdb.jdbc-user"),
                config.get("tierdb.jdbc-password"), pinTtl, Map.copyOf(fileIo));
    }
